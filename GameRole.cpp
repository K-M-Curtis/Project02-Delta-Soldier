...
bool GameRole::Init()
{
	...
	for (auto &r : players)
	{
		...
		cout << "Init surround players: " << role->mPlayerName << endl;
		//产生上线消息
		msg = MakeInitPosBroadcast();
		//注意发送给对方的protocol层
...
void GameRole::ProcSkillContact(pb::SkillContact * contact)
{
	...
	for (auto p : players)
	{
		...
		if (targetRole->hp <= 0)
		{
			...
			cout << "Player id: " << targetRole->mPlayerID << " name: " << targetRole->mPlayerName << " die in hp: " << targetRole->hp << endl;
...
void GameRole::ViewDisappear(std::list<AOI_Player*>& oldList, std::list<AOI_Player*>& newList)
{
	...
	//集合运算,差集
	std::set_difference(oldVec.begin(), oldVec.end(), newVec.begin(), newVec.end(), std::inserter(diff, diff.begin()));
...
void GameRole::ViewAppear(std::list<AOI_Player*>& oldList, std::list<AOI_Player*>& newList)
{
	...
	vector<AOI_Player*>diff;
	//集合运算,差集
	std::set_difference(newVec.begin(), newVec.end(), oldVec.begin(), oldVec.end(), std::inserter(diff, diff.begin()));
